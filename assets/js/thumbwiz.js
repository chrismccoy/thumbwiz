                            ...
                            function() {
                                if (typeof wp.media.frame.state().get('selection').first() != 'undefined') {
                                    ...
                                    if (jQuery('#attachments-' + attributes.id + '-thumbwiz-poster').val() === '') {
                                        ...
                                        jQuery('#attachments-' + attributes.id + '-thumbwiz-numberofthumbs').val(thumbwiz_L10n.generate_thumbs);
...
function thumbwiz_attachment_selected(attributes) {
    ...
    if (jQuery('.thumbwiz_redraw_thumbnail_box').length) {
        setTimeout(function() {
            thumbwiz_redraw_thumbnail_box(attributes.id)
        }, 5000);
    }
}
...
function thumbwiz_reveal_thumb_video(postID) {
    ...
    if (text.html() != thumbwiz_L10n.cantloadvideo) {
        ...
        jQuery('#thumb-video-' + postID + '-player').animate({
            opacity: 'toggle',
            height: 'toggle'
        }, 500);
        jQuery('#generate-thumb-' + postID + '-container').animate({
            opacity: 'toggle',
            height: 'toggle'
        }, 500);
            ...
            jQuery('#' + video_id).on(
                ...
                function() { // when the video is finished seeking
                    ...
                    if (typeof thumbnail_saved !== 'undefined' &&
                        ...
                        jQuery('#attachments-' + postID + '-thumb' + time_id).animate({
                            opacity: 'toggle',
                            height: 'toggle',
                            width: 'toggle'
                        }, 1000);
                        ...
                        if (thumbnail_saved.length > 0) {
                            ...
                            jQuery(cancelthumbdivID).animate({
                                opacity: 0,
                                height: 'toggle'
                            }, 500);
...
function thumbwiz_save_canvas_thumb(postID, time_id, total, index) {
    ...
    jQuery.ajax({
            ...
            url: ajaxurl,
            data: {
                action: "thumbwiz_save_html5_thumb",
        ...
        jQuery.post(
            ...
            function(data) {
                if (data.thumb_url) {
                    ...
                    setTimeout(function() {
                        thumbwiz_redraw_thumbnail_box(postID)
                    }, 5000);
                }
